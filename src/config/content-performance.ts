// ============================================================
// PERFORMANCE & TAF — Conteúdo da subpágina /performance
// Edite aqui todos os textos desta página
// ============================================================

export const PERFORMANCE = {
  hero: {
    eyebrow:     "TAF & Performance Esportiva",
    title:       "Preparação para o TAF",
    titleAccent: "Com dados, sem improviso.",
    description: "Preparação para o TAF (Teste de Aptidão Física) de concursos militares e policiais, além de avaliação e preparação para atletas e praticantes de esporte.",
    image:       "/fotos/taf.jpg",
    imageAlt:    "Avaliação de performance esportiva na MedFit",
    cta:         { label: "Agendar avaliação", href: "https://wa.me/5531995995428" },
    imagePosition: "center 40%",  // ← ajusta o enquadramento no mobile (ex: center_20%, center_30%)
  },

  // ─── ATESTADO MÉDICO DE APTIDÃO FÍSICA (concursos) ─────────
  atestado: {
    eyebrow: "Atestado Médico de Aptidão Física",
    title:   "Atestado médico conforme exigido pelo edital",
    body:    "Emitimos o atestado médico de aptidão física necessário para a etapa do TAF em concursos públicos, com avaliação médica e física de acordo com as exigências de cada edital.",
    instituicoes: [
      "PMMG",
      "Polícia Civil",
      "Corpo de Bombeiro Militar",
      "Guarda Municipal",
      "Polícia Penal",
      "e outros concursos",
    ],
  },

  features: {
    eyebrow: "O que avaliamos",
    title:   "Performance construída sobre dados reais.",
    body:    "Cada avaliação gera dados objetivos sobre força, resistência, velocidade e recuperação — base para um treinamento mais eficiente.",
    items: [
      {
        title:       "Teste de aptidão física (TAF)",
        description: "Avaliação específica para concursos militares, policiais e demais processos seletivos físicos.",
      },
      {
        title:       "Avaliação de força",
        description: "Dinamometria isométrica para mensurar força, assimetrias e desequilíbrios bilaterais.",
      },
      {
        title:       "Capacidade aeróbica",
        description: "Testes de VO₂máx, limiar anaeróbio e zonas de frequência cardíaca para prescrição precisa.",
      },
      {
        title:       "Composição corporal",
        description: "Análise de massa muscular, gordura e hidratação para obter os parâmetros iniciais e mensurar a evolução.",
      },
      {
        title:       "Plano de periodização",
        description: "Organização do treinamento em ciclos para pico de performance no momento certo.",
      },
      {
        title:       "Prevenção de lesões",
        description: "Identificação de desequilíbrios e fatores de risco antes que se tornem problemas.",
      },
    ],
  },

  process: {
    eyebrow: "Como funciona",
    title:   "Da avaliação à evolução mensurável.",
    steps: [
      {
        number:      "01",
        title:       "Anamnese detalhada",
        description: "Histórico esportivo, lesões anteriores, objetivos e data do evento ou prova.",
      },
      {
        number:      "02",
        title:       "Exames e testes",
        description: "Exames e testes físicos específicos para o esporte ou prova: força, velocidade, resistência.",
      },
      {
        number:      "03",
        title:       "Análise e plano",
        description: "Relatório detalhado com pontos fortes, fraquezas e plano de treinamento individualizado.",
      },
      {
        number:      "04",
        title:       "Acompanhamento",
        description: "Monitoramento do progresso com reavaliações periódicas e ajuste do planejamento.",
      },
    ],
  },

  cta: {
    title:  "Pronto para treinar com mais inteligência e menos desperdício?",
    body:   "Agende sua avaliação de performance e descubra onde concentrar seus esforços para evoluir de maneira mais eficiente.",
    button: { label: "Agendar pelo WhatsApp", href: "https://wa.me/5531995995428" },
  },
};
